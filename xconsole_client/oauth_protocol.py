...
After account signup (or with email/password), this module:
  ...
  3. Solves Cloudflare Turnstile via YesCaptcha or EzSolver
...
from . import grpcweb
from .solver import SolverType, create_solver, resolve_solver_provider
...
class ProtocolOAuthClient:
    ...
    def __init__(
        ...
        *,
        ...
        self.solver: Optional[SolverType] = solver
        if self.solver is None:
            provider = (solver_provider or "").strip() or None
            # Backward compatible: explicit YesCaptcha key still works.
            if self._yescaptcha_key and (
                not provider or resolve_solver_provider(provider) == "yescaptcha"
            ):
                try:
                    self.solver = create_solver(
                        provider="yescaptcha",
                        api_key=self._yescaptcha_key,
                        debug=debug,
                    )
                except Exception:
                    self.solver = None
                ...
                try:
                    self.solver = create_solver(provider=provider, debug=debug)
                except Exception:
                    self.solver = None
        try:
    ...
    def create_session(self, email: str, password: str, *, referer: str) -> Dict[str, Any]:
        ...
        if not self.solver:
            return {
                "ok": False,
                "error": "Turnstile solver required for CreateSession (set YESCAPTCHA_API_KEY or TURNSTILE_SOLVER=ezsolver + EZSOLVER_ENDPOINT)",
                "grpc_status": None,
                "session_jwt": None,
                "raw_fields": [],
            }
